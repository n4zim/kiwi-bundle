...
var Database = (function () {
    function Database(name, repositories) {
        ...
        databaseRequest.onerror = function () {
            logger_1.logger.logError(_this, "IndexDB error", event);
        };
        ...
        databaseRequest.onsuccess = function () {
            logger_1.logger.logSuccess(_this, "IndexedDB connected", event);
    ...
    Database.prototype.onUpgradeNeeded = function (database) {
        ...
        this.repositories.forEach(function (repository) {
            if (!database.objectStoreNames.contains(repository.name)) {
                ...
                logger_1.logger.logInfo(_this, "Created " + repository.name + " store");
            }
        });
    };
    Database.prototype.onSuccess = function (database) {
        ...
        this.repositories.forEach(function (repository) {
            ...
            logger_1.logger.logInfo(repository, "Repository for " + _this.name + " loaded");
            if (--check === 0) {
                logger_1.logger.logInfo(_this, _this.repositories.length + " repositor" + (_this.repositories.length === 1 ? "y" : "ies") + " has been loaded");
            }
        });
    };
    return Database;
