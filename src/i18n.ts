...
export function i18n(schema: i18nSchema, options: i18nOptions = {}): string {
  if (typeof schema === "number") { // NAME NUMBER
    return schema.toString()
  }
  if (typeof schema === "string") { // NAME STRING
    return fromString(schema, options)
  }
  if (typeof schema === "object") {
    if (Array.isArray(schema)) { // ARRAY
      return fromArray(schema, options)
    }
    const schemaAsQuery = schema as i18nQuery
    if (typeof schemaAsQuery.$ !== "undefined") {
      return fromQuery(schemaAsQuery, options)
    }
...
const fromAPerson = (person: NameField_ForAPerson, options: i18nOptions): string => {
  ...
  if (typeof person.middlename !== "undefined") {
    output += (output.length ? " " : "") + person.middlename
  }
  if (typeof person.lastname !== "undefined") {
    output += (output.length ? " " : "") + person.lastname
  }
  return i18n(output, options)
}
